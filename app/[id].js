...
const edit = () => {
  ...
  useEffect(() => {
    fetch(process.env.EXPO_PUBLIC_API_URL + "/" + params.id)
      .then((res) => res.json())
      .then(
        (result) => {
          setIsLoading(false);
  ...
  async function addHabit() {
    try {
      ...
      const response = await fetch(
        process.env.EXPO_PUBLIC_API_URL + "/" + params.id,
        {
          method: "PATCH",
          headers: {
  ...
  async function deleteHabit() {
    try {
      ...
      console.log("error deleting a habit", error);
    }
  }
    ...
    return (
      <View style={{ padding: 10, paddingTop: 50 }}>
        ...
        <Text style={{ fontSize: 20, marginTop: 10 }}>
          Edit <Text style={{ fontSize: 20, fontWeight: "500" }}>Habit</Text>
        </Text>
        <TextInput
          value={title}
